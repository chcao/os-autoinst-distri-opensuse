...
sub prepare_boot_params {
    ...
    push @params, 'inst.register_url=' . get_var('SCC_URL') if get_var('SCC_URL') && 
        (get_var('FLAVOR') =~ /^(Online.*|agama-installer)$/ || get_var('AGAMA_FORCE_REGISTER')) && 
        !(get_var('ISO') =~ /Leap/ && get_var('FLAVOR') =~ /^(Online.*|agama-installer)$/);
